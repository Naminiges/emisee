...
export const Profil = () => {
  ...
  const onsubmitProfile = async () => {
    ...
    try {
      ...
      await addDoc(userProfileCollectionRef, {
        user_id: JSON.parse(localStorage.getItem("currentUser")).uid,
        username: username,
        name: name,
        email: email,
  ...
  const onUpdateProfile = async () => {
    try {
      const currentUserUid = JSON.parse(
  ...
  useEffect(() => {
    // Ambil nilai user_id dari currentUser
    const currentUserUid = JSON.parse(localStorage.getItem("currentUser")).uid;

    // Cari data user yang sesuai dengan user_id dari currentUser
    const currentUserData = userprofile.find(
      (user) => user.user_id === currentUserUid
    );

    // Jika data user ditemukan, isi nilai input dengan data tersebut
    if (currentUserData) {
      ...
      // Anda mungkin perlu menambahkan logika tambahan untuk gambar profil jika ada
    }
  }, [userprofile]);
